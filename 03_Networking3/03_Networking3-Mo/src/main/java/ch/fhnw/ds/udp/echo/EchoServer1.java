	...
	public static void main(String[] args) throws Exception {
		try (DatagramSocket socket = new DatagramSocket(1234)) {
			...
			while (true) {
				...
				var msg = new String(packet.getData(), packet.getOffset(), packet.getLength(), StandardCharsets.US_ASCII);
				var response = String.format("%tT: %s", LocalTime.now(), msg);
				packet.setData(response.getBytes(StandardCharsets.US_ASCII));
				socket.send(packet);
			}
		}
	}
