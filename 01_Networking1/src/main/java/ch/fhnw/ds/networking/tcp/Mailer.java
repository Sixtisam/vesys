...
public class Mailer {
	...
	public static void main(String[] args) throws Exception {
		...
		send("HELO " + hostName);
		receive();
		send("MAIL FROM: <samuel.keusch@students.fhnw.ch>");
		receive();
		send("RCPT TO: <stefan.landoes@students.fhnw.ch>");
		receive();
		send("DATA");
		receive();
		send("To: stefan.landoes@students.fhnw.ch");
		send("Subject: Meeting");
		send("Hallo Stefan, komm doch rasch in mein Buero.");
		send(".");
		receive();
		s.close();
	}
